...
library(tidyverse)
library(ggplot2)
FTIRdata <- read.csv('AN06P27-3.csv', sep=',', header = TRUE) #Here the name of my file was "AN06P27-3.csv". ".csv" has to be in the end of the name in the script.
head(FTIRdata)

FTIRdata2<- filter(FTIRdata, `wavenumber`>=650 & `wavenumber`<=3900) # I only want to plot the data in the 600-4000 wavenumber range

head(FTIRdata2)
ggplot(FTIRdata2, aes(x=`wavenumber`, y=`Transmittance`)) +
  geom_line(colour="navy", size= 0.5)+ 
